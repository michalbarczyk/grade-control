...
from system.models import Teacher, Course, Student, Event, AcademicGrade
from system.views import append_sidebar


class EventCreateView(LoginRequiredMixin, CreateView):
    model = Event
    fields = ['title', 'description', 'weight', 'grade_type', 'max_score']
...
class EventDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for student in course.students.all():
            grade_model = event.get_grade_model()####
            event_grade = grade_model.objects.filter(owner=student, event=event).first()####
